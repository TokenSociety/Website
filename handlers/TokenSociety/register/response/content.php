...
function TokenSociety_register_response_content($params)
{
	// Do controller stuff here. Prepare variables
	$tabs = array("foo" => "bar");
	$description = "this is a description";
	Q_Response::addScript('js/pages/soon.js');
	
	// set meta tags
	$communityId = Users::communityId();
	$communityUser = Users_User::fetch($communityId);
	$text = Q_Text::get($communityId.'/content');
	$title = Q::text($text['soon']['Title'], array($communityId));
	$description = Q::text($text['soon']['Description'], array($communityId));
	$communityIcon = Q_Uri::interpolateUrl($communityUser->icon.'/400.png');
	$url = Q_Uri::url($communityId.'/soon');
	...
	return Q::view('TokenSociety/content/register.php', compact('tabs', 'description'));
}
