...
Q.Tool.define("TokenSociety/cool", function (options) {
	var tool = this;
	...
	Q.addStylesheet("css/TokenSociety.css", p.fill('stylesheet'), { slotName: 'TokenSociety' });
	Q.Text.get('TokenSociety/content', p.fill('text'));
...
{ // methods go here
	...
	refresh: function () {
		...
		tool.getMyStream(function (err, stream) {
			Q.Template.render('TokenSociety/cool/view', fields, function (err, html) {
				// replace all the HTML
				tool.element.innerHTML = html;
			
				// save some elements in an object
				elements.a = tool.element.getElementsByClassName('TokenSociety_foo');
				elements.b = tool.element.getElementsByClassName('TokenSociety_b');
...
Q.Template.set('TokenSociety/cool/view',
	'<div>'
	+	'<p class="TokenSociety_a">{{foo.bar}}</p>'
	+	'<input class="TokenSociety_sum" value="{{baz}}">'
